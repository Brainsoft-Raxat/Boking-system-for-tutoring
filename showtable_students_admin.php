...
$query="SELECT * FROM bookings, all_time_slots, logins WHERE bookings.student_id = '".$_POST['customer_id']."' and bookings.booking_time = all_time_slots.slot_id and bookings.dr_id=logins.id and bookings.dr_id=all_time_slots.dr_id  group by bookings.booking_id order By bookings.booking_date desc";
 ...
 { 
  ...
  <tr>
	...
	<th onclick="sortTable(0)" scope="col">Оплата</th>
  ...
  foreach($result as $loginrow)
  {
	...
	<td><input  type='checkbox' class='check' value='".$loginrow['booking_id']."'/></td>
